...
修改 `term_char_width()`，对 PUA 字符优先使用字体的实际字形宽度（通过 `win_char_width()` → `GetCharWidth32W()` 查询），而非依赖静态 Unicode 宽度表。

**涉及文件：**

| 文件 | 修改内容 |
|------|---------|
| `0.76b_My_PuTTY/terminal/terminal.h` | 将 `term_char_width()` 从 inline 函数改为声明 |
| `0.76b_My_PuTTY/terminal/terminal.c` | 添加 `term_char_width()` 实现，含 PUA 字体宽度查询 |
| `0.76_My_PuTTY/terminal.h` | 同上 |
...
这样无论 CJK 模式是否开启，PUA 字符的宽度都与字体实际字形匹配，消除了宽度不一致导致的显示问题。

### 相关代码路径

```
字符输入 → term_display_graphic_char() [terminal.c:4081]
         → term_char_width() [terminal.h:448 → terminal.c]
         → mk_wcwidth() / mk_wcwidth_cjk() [utils/wcwidth.c]

渲染时   → terminal.c:6965 检查 win_char_width() 与 ATTR_WIDE
         → 设置 ATTR_NARROW [terminal.c:6967]
         → window.c:6251 设置 FONT_NARROW
         → window.c:2857 字体宽度压缩为 (font_width+1)/2
         → window.c:6208 char_width = fnt_width（不乘2）
```
